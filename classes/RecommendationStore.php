...
/**
 * @file plugins/generic/recommendBySimilarity/classes/RecommendationStore.php
 *
...
namespace APP\plugins\generic\recommendBySimilarity\classes;
...
class RecommendationStore
{
    ...
    public function enqueueNew(array $contextIds, int $limit = 5000, int $queueLimit = 0): int
    {
        ...
        if ($queueLimit > 0) {
            $room = $queueLimit - DB::table('recommend_similarity_state')->count();
            if ($room <= 0) {
                return 0;
            }
        ...
        foreach (array_chunk($rows, 500) as $chunk) {
            DB::table('recommend_similarity_state')->insertOrIgnore($chunk);
        }
    ...
    {
        DB::table('recommend_similarity_state')->insertOrIgnore([[
    ...
    public function due(array $contextIds, int $limit, int $maxAgeDays): array
    {
        ...
        $rows = DB::table('recommend_similarity_state')
     ...
     */
    public function refresh(array $submissionIds, int $contextId, SimilarityFinder $finder, int $max): void
    {
        foreach ($submissionIds as $submissionId) {
            ...
            DB::transaction(function () use ($submissionId, $rows, $phrase) {
                DB::table('recommend_similarity_cache')->where('submission_id', $submissionId)->delete();
                foreach (array_chunk($rows, 500) as $insert) {
                    DB::table('recommend_similarity_cache')->insert($insert);
                }
                DB::table('recommend_similarity_state')
    ...
    {
        return DB::table('recommend_similarity_cache')
            ->where('submission_id', $submissionId)
            ->orderBy('seq')
            ->offset($offset)
            ->limit($limit)
    ...
    {
        return DB::table('recommend_similarity_cache')->where('submission_id', $submissionId)->count();
    }

    public function stateOf(int $submissionId): ?object
    {
        return DB::table('recommend_similarity_state')->where('submission_id', $submissionId)->first();
    }
